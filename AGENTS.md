...
- **`vscode/chat-panel-*` specs fail on this branch.** The harness (`source/vscode/chat-panel-harness.ts`) evals `plugins/vscode/media/chat-panel.js` in a VM sandbox but never loads `plugins/vscode/media/mention-utils.js` first (which `chat-panel.html` loads before it), so `globalThis.NanocoderMentionUtils` is undefined and every `chat-panel-thoughts`/`chat-panel-tool-cards` test throws. This is a pre-existing code/test issue, not an environment problem.
